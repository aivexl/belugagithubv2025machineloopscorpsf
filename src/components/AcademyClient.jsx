    ...
    const fetchArticles = async () => {
      try {
        ...
        const articlesWithImages = data.map(article => {
          const imageUrl = validateAndGetImageUrl(article.image, '/Asset/belugalogov3white.png');
      ...
      <div className="mb-6 flex justify-between items-center">
        <div className="text-left">
          ...
          <p className="text-gray-400 text-sm">
            Total: {articles.length} • With Images: {articles.filter(a => a.mainImage && a.mainImage !== '/Asset/belugalogov3white.png').length}
          </p>
        </div>
        <div className="flex items-center space-x-3">
          <button
            onClick={() => setShowDebugPanel(!showDebugPanel)}
            className={`px-3 py-2 rounded-md text-xs font-medium transition-colors ${showDebugPanel
                ...
                >
                  <div className="bg-duniacrypto-panel border border-gray-700 rounded-lg overflow-hidden hover:border-gray-600 transition-colors group h-full flex flex-col">
                    ...
                    <div className="aspect-video bg-gray-800 overflow-hidden">
                      ...
                      {article.mainImage && article.mainImage !== '/Asset/belugalogov3white.png' ? (
                        <ArticleThumbnail
